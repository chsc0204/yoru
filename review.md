...
## 12. (2026-08-28) 아이지킴이 — 하이브리드 제어(자율↔수동) 신규 구현 및 촬영 분류 체계 구축

### 핵심 개념
- **기존 컬럼/엔드포인트 재사용 설계**: 자율↔수동 모드 전환을 위해 새 테이블이나 새 상태 필드를 만드는 대신, 하드웨어 보고용으로 이미 있던 `robot_status.state`와 `PATCH /robot-status`를 그대로 재사용했다. "누가 쓰든 같은 컬럼"이라는 설계는 구현은 단순해지지만, 두 주체(웹/하드웨어)가 같은 값을 동시에 건드릴 수 있다는 위험도 같이 떠안는다는 걸 나중에 직접 겪었다(아래 버그 참고).
- **Next.js 16.3.1의 `beforeInteractive` 동작 변경**: `next/script`의 `beforeInteractive` 전략이 예전엔 파서를 막는 동기 스크립트로 즉시 실행됐는데, 이 버전에서는 `self.__next_s` 큐에 쌓아뒀다가 클라이언트 런타임 청크가 로드된 뒤에야 실행되는 방식으로 바뀌어 있었다. 다크모드 초기화 스크립트가 이 전략을 쓰고 있어서, body가 라이트 배경으로 먼저 페인트된 뒤 뒤늦게 다크로 바뀌는 FOUC가 생겼다 — 라이브러리 문서가 아니라 실제 렌더링된 HTML을 떠서 원인을 확인했다.
- **자동 판별 가능한 필드는 필수값으로 강제하지 않기**: 장애물 회피/수동조작 촬영을 구분하는 `capture_reason`을 추가하면서, 하드웨어가 매번 이 값을 명시하도록 강제하는 대신 `obstacle_group_id`/`event_id` 유무로 서버가 자동 판별하게 설계했다. 덕분에 기존 순찰·이벤트버스트 흐름의 하드웨어 코드는 한 줄도 안 바꿔도 새 분류 체계가 그대로 맞아떨어졌다.
- **서버 사이드 안전망 vs 근본 해결**: 웹의 모드 전환과 하드웨어의 자율 순찰 보고가 같은 엔드포인트를 공유하다 보니, 하드웨어가 아직 "수동조작중이면 자율 로직을 멈춘다"는 폴링 로직을 구현하기 전엔 하드웨어의 정기 보고가 웹이 설정한 수동 모드를 곧바로 덮어써 버렸다. 하드웨어 펌웨어 수정을 기다리지 않고 서버가 "수동조작중일 때는 자율 상태값으로의 전환을 무시"하는 가드를 추가해 즉시 방어하면서도, 이건 임시 안전망일 뿐 정식 해결은 하드웨어 쪽 폴링 구현이라는 걸 문서에 명시해뒀다.
- **완전한 스트리밍 대신 폴링형 준실시간**: 조작 중 화면을 진짜 영상 스트리밍으로 구현하지 않고, 1~2초 주기로 사진을 올려 Supabase Realtime으로 최신 한 장만 갱신하는 방식을 택했다. 실측한 촬영·전송 딜레이를 먼저 확인한 뒤 "이 정도면 조작감에 충분하다"고 판단하고 진행한 것이라, 체감 성능을 먼저 재보고 설계를 확정하는 순서가 중요했다.

### 실제 사용한 코드 예시
```tsx
// 다크모드 FOUC 수정: next/script 대신 파서를 막는 순수 <script>로 교체
<script
  id="theme-init"
...
```
```python
# 웹의 모드 전환과 하드웨어의 자율 보고가 충돌하지 않도록 하는 서버 안전망
AUTONOMOUS_ONLY_STATES = {"순찰중", "이동중", "장애물회피중"}

if current["state"] == "수동조작중" and state in AUTONOMOUS_ONLY_STATES:
    logger.warning("수동조작중 상태에서 자율 상태(%s) 보고를 무시했습니다.", state)
    return current  # 하드웨어 응답은 200으로 유지하되 실제 반영은 하지 않음
```

### 개발 과정
- 체크포인트(현관/책상/주방) 좌표를 1x1 테스트 공간에 맞게 재조정, 침대는 이번 발표 범위에서 제외
- 다크모드 새로고침 시 흰 화면이 깜빡이는 FOUC 버그 발견 및 수정
- 하이브리드 제어(자율↔수동) 신규 구현: DB에 `수동조작중` 상태와 `control_commands` 테이블 추가 → 서버에 명령 수신 API 추가 → 웹에 모드 전환 버튼 + 방향 D-패드(후진 중 좌/우회전 비활성화) 구현
- 로봇 상태 보고에 줄 내 진행률(`row_progress_percent`, 0~100%) 필드 추가
- 장애물 회피 촬영을 일반 순찰과 구분하는 `capture_reason`/`obstacle_group_id` 체계 구축 (순찰/이벤트버스트/장애물회피/수동조작 4종)
- 수동 조작 중 포토 스트리밍 기능 추가 — Gemini 분석은 비용 절약을 위해 스킵하도록 분기

### 팀 협업
- 웹 대시보드의 방향 버튼 클릭이 실제로 서버 → 하드웨어까지 이어져 로봇이 물리적으로 움직이는 것까지 B와 함께 실기기로 검증
- B가 실측으로 "로봇이 수동모드 진입 시 상태를 한 번만 보고한다"고 확인해줬는데, 실제로는 자율 순찰 루프 자체가 멈추지 않아 계속 상태를 덮어쓰고 있던 것으로 밝혀짐 — 한쪽의 관찰만으로 결론 내리지 않고 서버 로그·실시간 폴링으로 직접 재현해서 교차 확인한 게 원인을 좁히는 데 결정적이었다
- B가 제안한 "수동 조작 후 원래 순찰 위치로 물리적 복귀" 기능은, 현재 하드웨어로는 정밀한 위치 추적이 어렵다는 한계를 함께 검토한 뒤 이번 범위에서는 구현하지 않기로 결정

### 발견하고 해결한 버그
- **다크모드 FOUC**: `next/script`의 `beforeInteractive`가 이 Next.js 버전에서 즉시 실행되지 않고 클라이언트 런타임 로드 후로 지연되는 것이 원인 — 파서를 막는 순수 `<script>` 태그로 교체해 body 페인트 전에 테마가 확정되도록 수정
- **모드 전환 race condition**: 웹에서 "자율→수동" 버튼을 눌러도 수십 초 뒤 하드웨어의 자율 순찰 보고가 다시 덮어써서 마치 버튼이 안 먹는 것처럼 보였던 문제 — `수동조작중` 상태에서 들어오는 자율 상태값을 서버가 무시하도록 가드를 추가해 해결. 처음엔 프런트 코드 버그(비대칭 로직)를 의심했지만, 코드 대칭성 확인 → 서버 로그의 실제 브라우저 에러 확인 → 실시간 폴링으로 상태값이 저절로 바뀌는 것을 직접 잡아내는 순서로 범위를 좁혀갔다

### 실습 요약
- Supabase REST API를 SQL Editor 대신 활용해 좌표 UPDATE·마이그레이션 반영 여부·제약조건 동작까지 매번 직접 조회해서 검증
- 순찰/이벤트버스트/장애물회피/수동조작 4가지 `capture_reason` 케이스를 실제 요청으로 하나씩 보내 분류와 Gemini 분석 스킵 여부까지 확인
- 서버가 브라우저 콘솔 에러를 터미널로 그대로 전달해주는 Next.js dev 서버의 기능을 활용해, 실제 사용자가 겪은 네트워크 에러를 코드 줄 번호까지 특정해서 원인을 확인
- 의심되는 원인을 코드 검토로 끝내지 않고, 실제 상태값을 몇 초 간격으로 관찰하며 "무엇이, 언제, 무엇으로 덮어쓰는지"를 실측으로 재현해서 확정

---

## 오늘의 한 줄 정리
> 파일은 규칙(확장자)으로 자동 정리하고, 코드는 Git으로 버전을 기록하며, 게임은 "입력 → 상태 업데이트 → 그리기"의 반복 루프로 만들어진다. 그리고 FastAPI+SQLite로 실제 동작하는 백엔드를 만들고 Next.js 프론트와 REST API로 연결해, 완성된 풀스택 미니 프로젝트를 GitHub에 올려보는 것까지 경험했다. 외부 API(Open-Meteo)를 병렬로 호출하는 법과, Supabase 같은 BaaS로 실제 회원가입/게시글 작성이 동작하는 커뮤니티 서비스를 만드는 것까지 하루에 경험했다. 그리고 하드웨어(로봇)-서버-AI-DB-웹 대시보드를 전부 잇는 실제 팀 프로젝트(아이지킴이)에서, Gemini 구조화 출력·Supabase Realtime·이벤트 버스트 설계 같은 아키텍처 결정과, 필드명 오매핑·프롬프트 모호성·API 쿼터 소진처럼 실제로 부딪혀야만 보이는 버그들을 직접 찾아 고쳐보는 경험까지 이어갔다. 여기에 더해 RGB LED·화염 센서·초음파·서보/스테퍼·조이스틱·LCD I2C 같은 아두이노 센서/액추에이터를 하나씩 직접 다뤄보며, 컴파일 에러·업로드 실패·화면 미출력처럼 층위가 다른 문제를 각각 다른 방식(코드 줄 번호, 포트/케이블/리셋, 하드웨어 조절 나사)으로 좁혀나가는 디버깅 감각과, 센서 임계값 하나가 오탐/미탐을 가른다는 것도 몸으로 익혔다. 아이지킴이 프로젝트에서는 여기서 더 나아가, 기존 하드웨어 보고용 컬럼을 재사용해 자율↔수동 하이브리드 제어를 새로 구현하고 웹 버튼 클릭이 실제 로봇의 움직임으로 이어지는 것까지 실기기로 검증했으며, "두 주체가 같은 값을 공유하면 경쟁이 생긴다"는 걸 프런트 코드 의심 → 서버 로그 확인 → 실시간 관찰로 원인을 좁혀가며 직접 겪었다. 라이브러리 버전이 바뀌며 생긴 FOUC 버그를 문서가 아니라 실제 렌더링된 HTML로 확인해 고치고, 촬영 사유를 자동 판별하도록 설계해 기존 하드웨어 코드를 건드리지 않고도 새 분류 체계를 얹었으며, 완전한 스트리밍 대신 실측 딜레이를 먼저 재본 뒤 폴링형 준실시간으로 타협점을 찾는 결정까지, 하루 안에서 설계·구현·트러블슈팅·팀 의사결정을 한 바퀴 돌려보는 경험을 했다.
